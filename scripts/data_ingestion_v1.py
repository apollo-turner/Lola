...
class DataIngestion:
    ...
    def scan_folders(self, path):
        ...
        for root, dirs, files in os.walk(path):
            if "inbox" in root.lower() and "message_1.json" in files:
...
if __name__ == "__main__":
    ...
    data_ingestor = DataIngestion(base_path=r"C:\social-media-analysis\data\raw\facebook")
    df = data_ingestor.process_all_directories()

    if not df.empty:
        # Filename format: YYYY_MM_DD_HHMMSS
        timestamp = datetime.now().strftime('%Y_%m_%d_%H%M%S')
        processed_folder = r"C:\social-media-analysis\data\processed"
